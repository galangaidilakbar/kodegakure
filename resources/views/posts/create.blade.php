<x-guest-layout>
        ...
        <main class="container min-h-screen lg:my-5">
            <div class="bg-white rounded px-4 py-2.5 max-w-lg mx-auto lg:border">
                ...
                <div id="success_message"
                    class="hidden bg-green-100 text-green-700 px-4 py-2 rounded-lg mb-3 capitalize flex justify-center items-center space-x-2">
                ...
                <div class="mb-3">
                    ...
                    <input type="file" name="filename" id="filename"
                        class="mt-1 block w-full border border-gray-300 rounded-lg p-2.5 text-sm bg-gray-50 focus:ring-blue-500 focus:border-blue-500"
                        required multiple>
                </div>

                <div class="mb-3">
                    <x-label for="title" :value="__('Title')" />
                    <x-input id="title" class="block mt-1 w-full bg-gray-50" type="text" name="title"
                        :value="old('title')" required />
                </div>

                <div class="mb-3">
                    <x-label for="summary" :value="__('Summary')" />
                    <x-input id="summary" class="block mt-1 w-full bg-gray-50" type="text" name="summary"
                ...
                <div class="pb-3">
                    ...
                    <textarea id="description" name="description" rows="6"
                        class="resize-none mt-1 block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
    ...
    <script>
        ...
        if (token === null) {
            $("#store").removeClass('bg-blue-500 hover:bg-blue-600').addClass('bg-gray-500 cursor-not-allowed').prop(
                'disabled', true)
        ...
        const loadingSpinner =
            `<div class="border-4 border-white border-t-4 border-t-green-500 rounded-full w-6 h-6 animate-spin"></div>`
                ...
                .then(response => {
                    if (response.status === 201) {
                        $("#success_message").html(
                            `<span><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-green-700" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" /></svg></span> <div>${response.data.message}</div>`
                        );
                        $("#title").val('');
                        $("#summary").val('');
                        $('#filename').val('');
